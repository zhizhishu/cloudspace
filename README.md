...
The bundled mihomo core tracks `v1.19.27`, matching the Clash.Meta target generated by current Sub-Store availability scripts. The Docker build also runs the bundled core once so an incompatible binary fails during image publishing instead of surfacing later as refused local HTTP META test ports.
